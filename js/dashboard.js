...
const SSIDashboard = (() => {
  function render(area) {
    if (!SSIApp.hasRole('ADMIN')) { area.innerHTML='<div class="empty-state"><div class="icon">🔒</div><p>Access Denied</p></div>'; return; }
    ...
    const ordersThisMonth = st.orders.filter(o=>{
      const d=new Date(o.created_at); return d.getMonth()===thisMonth && d.getFullYear()===thisYear;
    });
    ...
    const revenueThisMonth = ordersThisMonth.filter(o=>o.status==='DISPATCHED')
    ...
    st.products.filter(p=>p.active && p.reorder_level>0).forEach(p=>{
      st.units.filter(u=>u.active).forEach(u=>{
        ...
        if (qty <= p.reorder_level) lowStock.push({ product: p.name, unit: u.name, qty, reorder: p.reorder_level });
      });
    });

    // Recent orders
    const recentOrders = [...st.orders].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).slice(0,5);
    ...
    area.innerHTML = `
      <div style="padding:0 0 24px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:24px;">
          ...
          ${statCard('🛒','Pending Orders',pendingOrders,'Awaiting dispatch','#d97706')}
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;flex-wrap:wrap;">
          ...
          <div class="card">
            ...
            ${lowStock.length===0
              ...
              : `<div style="display:flex;flex-direction:column;gap:8px;">${lowStock.slice(0,6).map(l=>`
          ...
          <div class="card">
            ...
            ${recentOrders.length===0
              ...
              : `<div style="display:flex;flex-direction:column;gap:8px;">${recentOrders.map(o=>{
                  ...
                  return `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;background:#f8fafc;border-radius:8px;">
                    <div>
                      <div style="font-size:13px;font-weight:600;color:#111827;">${o.order_no} ${o.urgent?'🚨':''}</div>
                      <div style="font-size:12px;color:#64748b;">${client?client.name:'—'} • ${SSIApp.dateFmt(o.created_at)}</div>
                    </div>
            ...
            <table>
              ...
              <tbody>
                ${st.products.filter(p=>p.active).map(p=>{
                  ...
                  const totalQty = stocks.reduce((a,b)=>a+b,0);
                  const isLow = p.reorder_level>0 && totalQty<=p.reorder_level;
                  return `<tr>
                    <td><strong>${p.name}</strong><br><span style="font-size:11px;color:#94a3b8;">${p.sku}</span></td>
                    ${stocks.map(q=>`<td style="text-align:center;font-weight:600;color:${q<=0?'#dc2626':q<=(p.reorder_level||0)?'#d97706':'#16a34a'};">${SSIApp.qtyFmt(q)}</td>`).join('')}
                ...
                }).join('') || '<tr><td colspan="10" style="text-align:center;color:#94a3b8;padding:20px;">No products added yet</td></tr>'}
              </tbody>
            </table>
          </div>
        </div>
